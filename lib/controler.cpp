...
void Controler::asyncRemoveRow(int row)
{
    auto i = m_model->index(row, 0);
    if (i.isValid()) {
        asyncRemove(i.data(TaskModel::RoleData).value<Task>());
    }
}
...
void Controler::setModel(TaskModel *model)
{
    if (m_model == model)
        return;
...
void Controler::setRepository(Repository *repository)
{
...
Repository *Controler::repository() const
{
    return m_repository;
}
